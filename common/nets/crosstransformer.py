...
class Block(nn.Module):
    ...
    def forward(self, q, k):
        ...
        # nq, attn= self.attn(self.norm1(q), self.norm2(k))
        # q = q + nq
...
class CrossTransformer(nn.Module):
    def __init__(self, in_chans1=512, in_chans2=512, num_token=12, depth=4, num_heads=4, mlp_ratio=4., norm_layer=nn.LayerNorm):
        super().__init__()
        self.pos_embed = nn.Parameter(torch.randn(1, num_token * num_token, in_chans1))
        self.pos_embed2 = nn.Parameter(torch.randn(1, 1 + 2 * num_token * num_token, in_chans2))
