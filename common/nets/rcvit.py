...
class AdditiveBlock(nn.Module):
    """
    """
    def __init__(self, dim, mlp_ratio=4., attn_bias=False, drop=0., drop_path=0., act_layer=nn.ReLU, norm_layer=nn.GELU):
        super().__init__()
...
class RCViT(nn.Module):
    def __init__(self, layers, embed_dims, mlp_ratios=4, downsamples=[True, True, True, True], norm_layer=nn.BatchNorm2d, attn_bias=False,
        ...
        for i in range(len(layers)):
            stage = Stage(embed_dims[i], i, layers, mlp_ratio=mlp_ratios, act_layer=act_layer, attn_bias=attn_bias, drop=drop_rate, drop_path_rate=drop_path_rate)
            ...
            if downsamples[i] or embed_dims[i] != embed_dims[i + 1]:
                ...
                network.append(
                    Embedding(patch_size=3, stride=2, padding=1, in_chans=embed_dims[i], embed_dim=embed_dims[i+1], norm_layer=nn.BatchNorm2d)
                )
    ...
    def init_weights(self, pretrained=None):
        ...
        if pretrained is None:
            ...
            print("load pretrained weights from ", pretrained)
            if os.path.isfile(pretrained):
                ...
                raise ValueError(f"Pre-trained weights file '{pretrained}' does not exist.")
...
# ======================================================================================================================
@register_model
def rcvit_ts(**kwargs): #2M
    model = RCViT(
        layers=[2, 2, 4, 2], embed_dims=[32, 48, 96, 160], mlp_ratios=4, downsamples=[True, True, True, True],
...
class RCViTBackboneNet(nn.Module):
    def __init__(self, model_type='s'):
        super(RCViTBackboneNet, self).__init__()
        if model_type == 'ts':
            self.model = rcvit_ts()
        elif model_type == 'xs':
            self.model = rcvit_xs()
    ...
    def init_weights(self, pretrained=None, model_type='s'):
        if pretrained is not None:
            pretrained_path = os.getenv('/data/users/fr/MyProjects/InterWild-main/output/model/')
            if model_type == 'ts':
                pretrained_file = os.path.join(pretrained_path, 'cas-vit-ts.pth')
            elif model_type == 'xs':
                pretrained_file = os.path.join(pretrained_path, 'cas-vit-xs.pth')
            elif model_type == 's':
                pretrained_file = os.path.join(pretrained_path, 'cas-vit-s.pth')
            elif model_type == 'm':
                pretrained_file = os.path.join(pretrained_path, 'cas-vit-m.pth')
            elif model_type == 't':
                pretrained_file = os.path.join(pretrained_path, 'cas-vit-t.pth')
            else:
                raise ValueError(f"Unsupported model type: {model_type}")

            if not os.path.isfile(pretrained_file):
                raise FileNotFoundError(f"Pre-trained weights file '{pretrained_file}' does not exist.")

            self.model.init_weights(pretrained=pretrained_file)
        else:
...
if __name__ == '__main__':
    backbone = RCViTBackboneNet('s')
    backbone.eval()
    with torch.no_grad():
        x = torch.rand((1, 3, 384, 384))
        out = backbone(x)
        print("rcvit backbone output shape", out.shape)
    
    # from torchinfo import summary
    # summary(backbone, input_size=(1, 3, 384, 384))
